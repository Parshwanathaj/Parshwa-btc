...
public class ReportGeneration {
	...
	public int[] totalscore(List<Student> studentList) {
		...
		for (int i = 0; i < studentList.size(); i++) {
			...
			while (i == j) {
				int total = studentList.get(i).getKannadaScore();
				total += studentList.get(i).getEnglishScore();
				total += studentList.get(i).getHindiScore();
				res[i] = total;
				j++;
	...
	public void getPassedOutStudents(List<Student> studentList) {
		for (int i = 0; i < studentList.size(); i++) {
			if (studentList.get(i).getKannadaScore() >= 50 && studentList.get(i).getEnglishScore() >= 50
					&& studentList.get(i).getHindiScore() >= 50) {
	...
	public void getStudentsForRetest(List<Student> studentList) {
		for (int i = 0; i < studentList.size(); i++) {
			if (studentList.get(i).getKannadaScore() < 50 || studentList.get(i).getEnglishScore() < 50
				...
				System.out.println(
						"STUDENT NAMES WHO FAILED IN THE EXAM (MARKS < 50) " + studentList.get(i).getStudentName());
			}
		}
	...
	public void getStudentRollNumber(List<Student> studentList) {
		...
		System.out.println(
				"Displays the roll numbers of the students on the class board in descending order of the total marks scored in the exam");
